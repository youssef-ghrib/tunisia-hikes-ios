...
class MyHikesTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("https://randonnee-tunisie.000webhostapp.com/randonnee/getRandonneesByUser.php?user_id=" + "\(getIdCurrentUser())").responseArray { (response: DataResponse<[Randonnee]>) in
            
            if response.response?.statusCode == 200 {
                
                self.hikes = response.result.value!
            }
    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            Alamofire.request("https://randonnee-tunisie.000webhostapp.com/randonnee/cancelHike.php", method: .post,parameters: parameters).responseJSON { response in
                
                if (response.response?.statusCode == 200) {
                    
                    randonnee.status = "canceled"
                    self.hikes[indexPath.row] = randonnee
                    self.tableView.reloadData()
                }
            }
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if(segue.identifier == "myHikesDetails"){
